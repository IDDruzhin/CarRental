...
namespace CarRental
{
    public class BDController : IBDController
    {
        ...
        {
            if (car.properties != null)
            {
                for (int i = 0; i < car.properties.Count; i++)
                {
                    car.properties[i] = ((IBDController)this).AddCarProperty(car.properties[i]);
                }
            }
            cars.InsertOne(car);
